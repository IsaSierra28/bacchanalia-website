# bacchanalia-website
# Bacchanalia — Restaurant & Pub Website

## About the Project

Bacchanalia is the website for a future restaurant and pub named after the ancient Roman festival — a celebration of wine, theater, and joy in honor of Bacchus, the god of wine. This site serves as the main online presence of the restaurant, giving visitors a sense of the concept, the atmosphere, and the experience they can expect.

## Motivation

This project is personal. It is built around the dream of someone very close to me — a chef who studied at the Stratford Culinary School in Ontario and has spent years working in professional kitchens. He now wants to open his own restaurant, a space that truly reflects his passion for food and for culture, especially the Roman world. The name Bacchanalia captures exactly the vibe he envisions: festive, a little theatrical, elegant but welcoming.

My goal is to help him bring that vision to life online, while putting into practice everything I have been learning in my Frontend Development course.

## Pages

- **Home** — Hero section with the restaurant name, tagline, and a call-to-action button
- **About** — The chef's story, his training, and the inspiration behind the name
- **Menu** — A preview of the food and drinks the restaurant will offer
- **Gallery** — Photos of the food, the space, and the overall atmosphere
- **Team** — Short profiles of the people behind the restaurant
- **Contact** — Address, hours, reservation form, and contact information

## Design

The design aims for something neutral and elegant — dark tones like charcoal and off-white with gold details that feel classy and a little ancient. Serif fonts for titles, clean sans-serif for body text, and moody, well-lit photography. The layout is simple and spacious, with card grids and dark sections as a visual foundation.

## Possible Domain Names

- `www.bacchanalia.ca` — short, memorable, directly tied to the name
- `www.bacchanaliarestaurant.com` — more descriptive, makes the concept clear
- `www.dine-bacchanalia.com` — friendlier, feels like an invitation

## Built With

- HTML5
- Bootstrap 5.3.3 (via CDN)

## Course Information

**Student:** Isabela Sierra Chinchilla  
**Course:** Frontend Development  
**Teacher:** Tiago Bortoletto Vaz  
**College:** Vanier College — Winter 2026


